...
export async function syncChats(): Promise<{ synced: number; errors: string[] }> {
  ...
  try {
    ...
    const result = await runLarkCli(['im', 'chats', 'list', '--format', 'json', '--page-all']) as LarkChatListResponse;
    ...
    for (const chat of items) {
      ...
      db.prepare(`
        INSERT INTO chats (chat_id, name, chat_type, avatar, is_monitoring, auto_reply, has_alert, last_active_at, updated_at)
